...
- Treat final release-note articles as independent `LIVE` patch sources. Never fold their values into the preceding PTR source; retain the PTR entry separately for its revision trail. Within a final article source, explicit `increased to … (was …)`, `reduced to … (was …)`, and `adjusted from … to …` wording remains an absolute replacement, not a cumulative multiplier. A qualitative replacement clears an obsolete numeric baseline.
